...
const AlertDialog = ({
    ...
    <AlertDialogContext.Provider
        value={{ variant, styleVariant, withIcon, customIcon, separatedHeader, separatedFooter }}
    >
...
>(({ className, children, ...props }, ref) => {
    ...
    React.Children.forEach(children, (child) => {
        if (React.isValidElement(child)) {
             if (typeof child.type !== "string" && (child.type as React.FC & { displayName?: string }).displayName === AlertDialogTitle.displayName) {
                titleElements.push(child);
            } else if (typeof child.type !== "string" && (child.type as React.FC & { displayName?: string }).displayName === AlertDialogDescription.displayName) {
    ...
    if (separatedHeader) {
            ...
            >
                ...
                <div className={cn(
                    ...
                    <AlertDialogPrimitive.Cancel
                        className={cn(
                            buttonVariants({ variant: "ghost", size: "icon", className: "!ml-4 !h-7 !w-7 flex-shrink-0" }),
        ...
        return (
            <div
                ref={ref}
                className={cn(alertDialogHeaderVariants({ variant, styleVariant }), className)}
                {...props}
            >
                {withIcon && bodyIcon && (
                    <div
...
>(({ className, children, ...props }, ref) => {
    ...
    const inlineIcon = customIcon ?? (inlineIconVariant ? variantIcons[inlineIconVariant] : null);
        ...
        >
              ...
              {showInlineIcon &&
                ...
                React.cloneElement(
                    ...
                    { className: cn("h-5 w-5 flex-shrink-0", (inlineIcon as React.ReactElement<{ className?: string }>).props
                    .className) }
                )}
...
>(({ className, ...props }, ref) => {
    ...
    return (<div ref={ref} className={cn(alertDialogFooterVariants({ styleVariant }), separatedFooter ? "border-t -mx-6 -mb-6 px-6 py-3" : "", className)} {...props} />);
});
